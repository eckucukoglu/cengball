...
public class XFactor extends Team {
	private static String teamName = "Juggernauts";
	private static Color teamColor1 = new Color(0, 0, 0), teamColor2 = new Color(238, 44, 44);
	private static String[] names = {"Hasan", "Georghe", "Arif", "Bulent", "Cladio"};
	
	private int PREDICTION_DISTANCE = 20, MARKING_DISTANCE = 6, SHOOT_DISTANCE = 30, SAFE_DISTANCE = 15, SAFE_RANGE = 10;
	private double SAFE_ANGLE = Math.PI / 6.0, FORMATION_DISTANCE = 50;
		...
		if ( owner == null ) {		// Ball is free
			...
			if ( owner.getTeamID() == this.getId() ) {		// I have the ball
				...
				if ( pToPass.getId() == owner.getId() || ownerState > totalBestState ) {	// Owner will have the ball
					...
					double ownerAngle = owner.getDirectionVector().getPosAngle();
					double shootAngle = shootPoint.subtract(owner.getPosition()).getPosAngle();
					
					if ( shootPoint.measureDistance(owner.getPosition()) < SHOOT_DISTANCE && checkViewAngle(ownerAngle,shootAngle) ) {
					...
					double ownerAngle = owner.getDirectionVector().getPosAngle();
					double passAngle = pToPass.getPosition().subtract(owner.getPosition()).getPosAngle();
					if ( checkViewAngle(ownerAngle, passAngle) ) {
				...
				lastBallAction = null;
				targetPlayer = null;
				manToManMarking(nextAssets);
			}
		}
	}
	
	private boolean checkViewAngle(double angle1, double angle2) {
	...
	private void moveWithBall(Player owner) {
		...
		double goalAngle = assets.getClosestPoint(opponentGoalPost, owner.getPosition()).subtract(owner.getPosition()).getPosAngle();
	...
	private vector2D findIntersectionPoint(Ball	ball, Player player) {
		...
		double angle = ball.getPosition().subtract(player.getPosition()).getPosAngle() - ball.getSpeed().getPosAngle();
	...
	private double normalizeDistance(double distance) {
		...
		return distance / FORMATION_DISTANCE;
	}
				...
				if ( getPlayerType(supporter) > 0 ) {	// Attacker
					if ( tactic < 0 ) {
						for(int i = 1; i < supportPoints.size(); i++) {
							...
							vector2D ocp = new vector2D(opponentGoalPost.getCenterX(), opponentGoalPost.getCenterY());
							double postDistance1 = normalizeDistance(supporter.getPosition().measureDistance(ocp));
							...
							double postDistance2 = normalizeDistance(supportPoints.get(i).measureDistance(ocp));
							double distance1 = normalizeDistance(supportPoint.measureDistance(formationPoint));
							double distance2 = normalizeDistance(supportPoints.get(i).measureDistance(formationPoint));
							if ( 17*(postDistance2-postDistance1) + 5 * (distance2 - distance1) + 5*(playersAround - numberOfPlayersAround) + 7*(playersOnLine - numberOfPlayersOnLine) < 0 ) {
						...
						for(int i = 1; i < supportPoints.size(); i++) {
							...
							double distance1 = normalizeDistance(supportPoint.measureDistance(formationPoint));
							double distance2 = normalizeDistance(supportPoints.get(i).measureDistance(formationPoint));
							if ( 17*(postDistance2-postDistance1) + 5 * (distance2 - distance1) + 9*(playersAround - numberOfPlayersAround) + 7*(playersOnLine - numberOfPlayersOnLine) < 0 ) {
								supportPoint = supportPoints.get(i);
							}
						}
					}
					...
					if ( tactic > 0 ) {
						...
						for(int i = 1; i < supportPoints.size(); i++) {
							...
							double distance1 = normalizeDistance(supportPoint.measureDistance(formationPoint));
							double distance2 = normalizeDistance(supportPoints.get(i).measureDistance(formationPoint));
							if ( 17*(postDistance2-postDistance1) + 5 * (distance2 - distance1) + 7*(playersAround - numberOfPlayersAround) + 9*(playersOnLine - numberOfPlayersOnLine) < 0 ) {
								supportPoint = supportPoints.get(i);
							}
						}
					}
